            ...
            <div class="grid gap-3 md:grid-cols-2">
              <div class="rounded-xl bg-slate-50 p-4">
                ...
                <div class="mt-3 space-y-2 text-sm text-stone-800">
                  ...
                  <p
                    v-if="!configForm.listen_addrs.length"
        ...
        >
          <div class="space-y-5">
            <div class="flex flex-wrap gap-3">
              ...
              <StatusBadge
                :text="
                  stats?.http3_listener_started ? '监听已启动' : '监听未启动'
                "
      ...
      >
        ...
        >
          ...
          <div class="flex flex-wrap gap-3">
            <StatusBadge
              :text="
                configForm.http_inspection_enabled
                  ? 'HTTP 检测开启'
                  : 'HTTP 检测关闭'
              "
              :type="configForm.http_inspection_enabled ? 'success' : 'warning'"
            />
            <StatusBadge
              :text="
                configForm.http2_enabled ? 'HTTP/2 已启用' : 'HTTP/2 未启用'
              "
              :type="configForm.http2_enabled ? 'info' : 'muted'"
            />
          </div>
        </div>
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.http_inspection_enabled"
                type="checkbox"
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.http2_enabled"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
              />
            </span>
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.bloom_enabled"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
              />
            </span>
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.upstream_healthcheck_enabled"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
              />
            </span>
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.bloom_false_positive_verification"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
                :disabled="!configForm.bloom_enabled"
              />
            </span>
            <span class="mt-2 block text-xs leading-6 text-slate-500"
          ...
          >
            <span class="flex items-center justify-between gap-3">
              ...
              <input
                v-model="configForm.http3_enabled"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
              />
            </span>
          ...
          >
            ...
            <select
              v-model="configForm.runtime_profile"
          ...
          >
            ...
            <select
              v-model="configForm.upstream_failure_mode"
              class="mt-3 w-full rounded-[16px] border border-slate-200 bg-white px-3 py-2.5 text-sm outline-none transition focus:border-blue-500"
            >
        ...
        <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <label class="text-sm text-stone-700 md:col-span-2">
            TCP 上游地址
            <input
              v-model="configForm.upstream_endpoint"
              type="text"
          ...
          <label class="text-sm text-stone-700">
            最大请求体大小
            <input
              v-model.number="configForm.max_request_size"
              type="number"
              min="1024"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            首字节超时(ms)
            <input
              v-model.number="configForm.first_byte_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            空闲读取超时(ms)
            <input
              v-model.number="configForm.read_idle_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            TLS 握手超时(ms)
            <input
              v-model.number="configForm.tls_handshake_timeout_ms"
              type="number"
              min="500"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            代理连接超时(ms)
            <input
              v-model.number="configForm.proxy_connect_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            代理写超时(ms)
            <input
              v-model.number="configForm.proxy_write_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            代理读超时(ms)
            <input
              v-model.number="configForm.proxy_read_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            健康检查间隔(s)
            <input
              v-model.number="configForm.upstream_healthcheck_interval_secs"
              type="number"
              min="1"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            健康检查超时(ms)
            <input
              v-model.number="configForm.upstream_healthcheck_timeout_ms"
              type="number"
              min="100"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            HTTP/2 最大并发流
            <input
              v-model.number="configForm.http2_max_concurrent_streams"
              type="number"
              min="1"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700">
            HTTP/2 最大帧
            <input
              v-model.number="configForm.http2_max_frame_size"
              type="number"
              min="1024"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700 md:col-span-2">
            HTTP/2 初始窗口
            <input
              v-model.number="configForm.http2_initial_window_size"
              type="number"
              min="1024"
              :class="numberInputClass"
            />
          ...
          <label class="text-sm text-stone-700 md:col-span-2">
            ...
            >
              <input
                v-model="configForm.http2_enable_priorities"
                type="checkbox"
                class="h-4 w-4 accent-blue-600"
              />
        ...
        <div class="mt-3 border-t border-slate-200 pt-6">
          ...
          >
            ...
            <div class="flex flex-wrap gap-3">
              <StatusBadge
                :text="
                  configForm.http3_enabled ? 'HTTP/3 已启用' : 'HTTP/3 未启用'
                "
                :type="configForm.http3_enabled ? 'success' : 'muted'"
              />
              <StatusBadge
                :text="
                  configForm.http3_enable_tls13 ? 'TLS1.3 开启' : 'TLS1.3 关闭'
                "
                :type="configForm.http3_enable_tls13 ? 'info' : 'warning'"
              />
            </div>
          </div>
          ...
          <div class="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
            <label class="text-sm text-stone-700 md:col-span-2">
              HTTP/3 监听地址
              <input
                v-model="configForm.http3_listen_addr"
                type="text"
            ...
            <label class="text-sm text-stone-700">
              最大并发流
              <input
                v-model.number="configForm.http3_max_concurrent_streams"
                type="number"
                min="1"
                :class="numberInputClass"
              />
            ...
            <label class="text-sm text-stone-700">
              空闲超时(s)
              <input
                v-model.number="configForm.http3_idle_timeout_secs"
                type="number"
                min="1"
                :class="numberInputClass"
              />
            ...
            <label class="text-sm text-stone-700">
              MTU
              <input
                v-model.number="configForm.http3_mtu"
                type="number"
                min="1200"
                max="1500"
                :class="numberInputClass"
            ...
            <label class="text-sm text-stone-700">
              最大帧大小
              <input
                v-model.number="configForm.http3_max_frame_size"
                type="number"
                min="65536"
                :class="numberInputClass"
              />
            ...
            <label class="text-sm text-stone-700">
              QPACK 表大小
              <input
                v-model.number="configForm.http3_qpack_table_size"
                type="number"
                min="1024"
                :class="numberInputClass"
              />
            ...
            <label class="text-sm text-stone-700 md:col-span-2">
              证书路径
              <input
                v-model="configForm.http3_certificate_path"
                type="text"
            ...
            <label class="text-sm text-stone-700 md:col-span-2">
              私钥路径
              <input
                v-model="configForm.http3_private_key_path"
                type="text"
            ...
            <label class="text-sm text-stone-700">
              ...
              >
                <input
                  v-model="configForm.http3_enable_connection_migration"
                  type="checkbox"
                  class="h-4 w-4 accent-blue-600"
                />
            ...
            <label class="text-sm text-stone-700">
              ...
              >
                <input
                  v-model="configForm.http3_enable_tls13"
                  type="checkbox"
                  class="h-4 w-4 accent-blue-600"
                />
      ...
      <section class="grid gap-4 xl:grid-cols-[1fr_1fr]">
        <CyberCard title="最近 L7 事件" sub-title="只展示七层安全事件">
          <div class="space-y-4">
            ...
            >
              ...
              <div
                class="mt-3 grid gap-2 text-sm text-stone-700 md:grid-cols-2"
              >
        ...
        <CyberCard title="L7 规则摘要" sub-title="规则中心中的七层策略概览">
          <div class="grid gap-4 md:grid-cols-3">
            <div class="rounded-xl bg-slate-50 p-4">
              <p class="text-xs tracking-wide text-slate-500">七层规则总数</p>
