...
| `src/l7/cc_guard/runtime.rs` | survival fast path 对低风险稳定身份请求跳过 ip / route / site 级 hot cache；full L7 CC 计数前也允许 verified normal pass；survival fast block 不再持久化真实 IP 到本地 blocked IP 列表 |
...
| `src/l7/cc_guard/tests.rs` | 增加低风险身份不被 ip/site hot cache 误伤、API 伪造身份不绕过、混合 Accept 头仍识别为正常 document、survival fast block 不持久化 IP 的单测 |
...
| 是否已经解决混合正常流量 | 60 秒短测过第一目标；180 秒长测已达到 `95.86%` / `97.55%` / `98.84%` |
| 是否影响 API 攻击防护 | 单测确认 API 伪造身份不会绕过 fast block |
| 下一步优先级 | 清理/过期历史 blocked IP 后做 600 秒 95/5、90/10、80/20 复核，再决定是否继续冲更高 open-loop 边界 |

## 6. 下一步建议

1. 清空或设置较短 TTL 过期历史 local blocked IP，再重跑 180 秒确认剩余 `blocked_client_ip` 是否归零。
2. 做 600 秒 95/5、90/10、80/20 混合长测；通过后才宣称生产混合口径更稳定。
3. 前端 Dashboard 已有攻击过程时间序列，后续可接入 early defense / L7 drop reason 分布。
4. 继续保持 API 不绕过，避免为了正常 document 保活把 API 攻击也放进来。
5. 如果 600 秒通过，再用专业 open-loop 工具复核 2500+ 边界。
