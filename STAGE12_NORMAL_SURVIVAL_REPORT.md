...
在 2200 RPS 混合 600 秒过线后，继续做 2500+ 边界复核。

纯攻击 advanced open-loop：

| 场景 | target RPS | 实际发送 RPS | effective TPS | send errors | CPU avg / max | 内存峰值 | OOM | 队列积压 |
|---|---:|---:|---:|---:|---:|---:|---|---:|
| advanced / 120s | 2500 | 2488.87 | 2488.40 | 0 | 138.88% / 161.74% | 59.8 MiB | false | 0 |
| advanced / 120s | 2600 | 2571.52 | 2571.06 | 0 | 143.63% / 157.97% | 62.5 MiB | false | 0 |
| advanced / 600s | 2600 | 2594.78 | 2594.69 | 62 | 152.76% / 171.72% | 113.4 MiB | false | 0 |

结论：

```text
纯攻击高级 CDN CC 已通过 2500+ open-loop 边界复核。
2600 target / 600s 下 effective TPS 为 2594.69，无 OOM，SQLite 队列为 0。
```

混合 95/5 探测：

| 场景 | target RPS | 实际发送 RPS | 正常请求 | 正常 200 | 正常成功率 | p95 | p99 | `blocked_client_ip` | `proxy_failures` |
|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|
| 95/5 / 180s | 2500 | 2474.27 | 21851 | 21851 | 100.00% | 178.07ms | 225.38ms | 0 | 0 |
| 95/5 / 180s | 2600 | 2575.28 | 23060 | 23060 | 100.00% | 182.62ms | 227.79ms | 0 | 0 |

结论：

```text
95/5 混合流量已完成 2500+ 短测探测。
2600 target / 180s 下实际发送 2575.28 RPS，正常成功率 100.00%，blocked_client_ip=0，proxy_failures=0。
混合 2500+ 还需要补 600 秒长测和 90/10、80/20 比例，暂不把它等同于完整生产复杂口径。
```
...
| 下一步优先级 | 补混合 2500+ 的 600 秒验证，扩展更多真实 IP / 正常 IP 组合，并补前端 reason 分布可视化 |
