...
因此，在当前 2 核 / 512MB 测试口径下，可以说高级 CDN CC 的 2000+ TPS 已同时覆盖纯攻击和混合正常业务流量的第一目标。
更高阶口径仍待验证：2500+ open-loop、更复杂正常路径比例、更大真实 IP / 正常 IP 组合。
```
...
| Stage 12 | 正常用户保活第一目标达成；修复 survival fast / hard block 持久化 IP 误伤后，600s 95/5、90/10、80/20 均达到 `100.00%` |
...
```text
继续 Stage 12，按 STAGE12_NORMAL_SURVIVAL_REPORT.md 的下一步建议，先做 2500+ open-loop 边界复核，再扩大混合矩阵和前端 reason 分布可视化。
```
