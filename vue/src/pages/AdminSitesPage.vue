...
const currentLocalSite = computed(() =>
  editingLocalSiteId.value === null
    ? null
    : (localSites.value.find((item) => item.id === editingLocalSiteId.value) ??
      null),
)
        ...
        >
          <div class="flex flex-wrap gap-2">
            ...
            >
              <PlugZap
                :size="14"
                :class="{ 'animate-pulse': actions.testing }"
              />
          ...
          <div class="flex flex-wrap gap-2">
            ...
            <StatusBadge
              :text="
                sitesLoadedAt
                  ? `远端已读取 ${formatNumber(sites.length)} 条`
                  : '远端站点未读取'
              "
      ...
      <template v-else>
        <section class="rounded-2xl border border-slate-200 bg-white shadow-sm">
          ...
          <div v-else class="overflow-x-auto">
            <table
              class="w-full min-w-[1420px] text-left text-sm text-slate-700"
            >
              <thead
                  ...
                  <td class="px-4 py-2">
                    <div v-if="row.local_present" class="space-y-1.5">
                      <div class="flex flex-wrap items-center gap-1.5">
                        ...
                        <StatusBadge
                          :text="
                            row.local_upstreams.length ? '已配上游' : '未配上游'
                          "
                          :type="
                            row.local_upstreams.length ? 'info' : 'warning'
                          "
                          compact
                        />
                      </div>
                      <div class="grid gap-1 text-xs text-slate-500">
                        ...
                        <p
                          class="truncate"
                          :title="row.local_hostnames.join(' / ')"
                        >
                          Hostnames：{{
                        ...
                        <p
                          class="truncate"
                          :title="row.local_upstreams.join(' / ')"
                        >
                          Upstream：{{
                  ...
                  <td class="px-4 py-2">
                    <div v-if="row.remote_present" class="space-y-1.5">
                      ...
                      <div class="grid gap-1 text-xs text-slate-500">
                        ...
                        <p
                          class="truncate"
                          :title="row.remote_upstreams.join(' / ')"
                        >
                          Upstream：{{
                            row.remote_upstreams.length
                    ...
                    <div class="flex flex-wrap items-center gap-2">
                      ...
                      >
                        <RefreshCw
                          :size="14"
                          :class="{
                            'animate-spin': rowActionPending(row, 'pull'),
                          }"
                        />
                      ...
                      >
                        <RefreshCw
                          :size="14"
                          :class="{
                            'animate-spin': rowActionPending(row, 'push'),
                          }"
                        />
