...
| 攻击 + 正常业务混合流量下 2000+ | 短测已过第一目标 | Stage 12 的 95/5、90/10、80/20 / 2200 RPS / 60s 正常成功率分别为 `95.76%`、`95.10%`、`94.78%`；仍需 180s / 600s 长测 |

因此，准确说法是：

```text
当前已经在 2 核 / 512MB 环境下，用 multi closed-loop 和 open-loop 口径验证了纯攻击高级 CDN CC 超过 2000 TPS。
Stage 12 已把 95/5、90/10、80/20 混合短测正常成功率提升到 90%+，但还不能宣称“生产复杂场景下高级 CDN CC 已全面稳定 2000+ TPS”，因为 80/20 仍略低于 95% 线，且 180s / 600s 长时间混合流量还没有完成验证。
```
...
| Stage 12 | 正常用户保活第一目标达成：normal-only 100 RPS `96.47%`、200 RPS `95.75%`；95/5、90/10、80/20 混合短测分别为 `95.76%`、`95.10%`、`94.78%`，仍需长测 |
...
1. 补 normal-only 50 RPS 阶梯点，并保留 100/200 RPS 作为后续回归基线。
2. 重跑 95/5、90/10、80/20 的 180s / 600s 混合长测，目标正常成功率稳定 95%+，再继续优化到 99%。
...
```text
继续 Stage 12，按 STAGE12_NORMAL_SURVIVAL_REPORT.md 的下一步建议，补 180s / 600s 长时间混合验证，并继续压低 80/20 下的剩余无响应。
```
