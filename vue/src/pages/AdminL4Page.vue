...
import { Activity, Ban, Database, RefreshCw, Save, ServerCog, Shield } from 'lucide-vue-next'
...
const refreshAll = async (showLoader = false) => {
  ...
  try {
    const [configPayload, statsPayload] = await Promise.all([fetchL4Config(), fetchL4Stats()])
...
const saveConfig = async () => {
  ...
  try {
    configForm.connection_rate_limit = clampInteger(configForm.connection_rate_limit, 1, 1_000_000, 100)
    configForm.syn_flood_threshold = clampInteger(configForm.syn_flood_threshold, 1, 1_000_000, 50)
    configForm.max_tracked_ips = clampInteger(configForm.max_tracked_ips, 1, 1_000_000, 4096)
    configForm.max_blocked_ips = clampInteger(configForm.max_blocked_ips, 1, 1_000_000, 1024)
    configForm.state_ttl_secs = clampInteger(configForm.state_ttl_secs, 60, 86_400, 300)
  ...
  <AppLayout>
    ...
    <div v-if="loading" class="flex h-72 items-center justify-center">
      <div class="flex flex-col items-center gap-4 rounded-[28px] border border-white/80 bg-white/75 px-8 py-10 shadow-cyber">
        ...
        <p class="text-sm tracking-[0.2em] text-cyber-muted">正在载入 L4 管理面板</p>
      </div>
    </div>
    ...
    <div v-else class="space-y-8">
      ...
      <section class="rounded-[34px] border border-white/85 bg-[linear-gradient(140deg,rgba(255,250,244,0.92),rgba(244,239,231,0.96))] p-7 shadow-[0_26px_80px_rgba(90,60,30,0.10)]">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div class="max-w-3xl">
            <p class="text-sm tracking-[0.22em] text-cyber-accent-strong">L4 管理</p>
            <h2 class="mt-3 font-display text-4xl font-semibold text-stone-900">四层防护策略与运行态联动面板</h2>
            <p class="mt-4 text-sm leading-7 text-stone-700">
              这里把 L4 的运行统计和持久化配置放到一起看：上半部分反映当前运行中的检测实例，下半部分是写入数据库的配置，保存后需要重启服务才会加载新参数。
            </p>
          </div>
      ...
      <section class="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        <CyberCard title="L4 防护配置" sub-title="保存到数据库后，重启服务即可让新配置接管运行时实例。">
          <div class="grid gap-5 md:grid-cols-2">
            <label class="rounded-[24px] border border-cyber-border/60 bg-cyber-surface-strong p-4 text-sm text-stone-700">
              <div class="flex items-center justify-between gap-4">
                <div>
                  <p class="font-medium text-stone-900">启用 DDoS 防护</p>
                  <p class="mt-1 text-xs leading-6 text-cyber-muted">关闭后仍会保留页面配置，但运行时不会做 DDoS 判定。</p>
                </div>
                <input v-model="configForm.ddos_protection_enabled" type="checkbox" class="h-5 w-5 accent-[var(--color-cyber-accent)]" />
              </div>
            </label>
            ...
            <label class="rounded-[24px] border border-cyber-border/60 bg-cyber-surface-strong p-4 text-sm text-stone-700">
              <div class="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p class="mt-1 text-xs leading-6 text-cyber-muted">额外使用更长窗口观测持续连接洪泛。</p>
                </div>
                <input v-model="configForm.advanced_ddos_enabled" type="checkbox" class="h-5 w-5 accent-[var(--color-cyber-accent)]" />
              </div>
            </label>

            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">每秒连接速率阈值</span>
              <input v-model.number="configForm.connection_rate_limit" type="number" min="1" step="1" :class="numberInputClass" />
              <p class="mt-2 text-xs text-cyber-muted">超过阈值后，连接限流器会拒绝来源地址的新连接。</p>
            </label>

            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">SYN / 突发阈值</span>
              <input v-model.number="configForm.syn_flood_threshold" type="number" min="1" step="1" :class="numberInputClass" />
              <p class="mt-2 text-xs text-cyber-muted">用于判定 1 秒窗口内是否出现连接洪泛。</p>
            </label>

            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">跟踪 IP 上限</span>
              <input v-model.number="configForm.max_tracked_ips" type="number" min="1" step="1" :class="numberInputClass" />
              <p class="mt-2 text-xs text-cyber-muted">连接跟踪器能同时维护的来源地址数量。</p>
            </label>

            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">封禁表上限</span>
              <input v-model.number="configForm.max_blocked_ips" type="number" min="1" step="1" :class="numberInputClass" />
              <p class="mt-2 text-xs text-cyber-muted">本地限流器允许同时保留的封禁 IP 数量。</p>
            </label>

            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">状态保留时长（秒）</span>
              <input v-model.number="configForm.state_ttl_secs" type="number" min="60" step="1" :class="numberInputClass" />
              <p class="mt-2 text-xs text-cyber-muted">连接窗口、限流计数和过期封禁的清理周期参考值。</p>
            </label>
            ...
            <label class="text-sm text-stone-700">
              ...
              <p class="mt-2 text-xs text-cyber-muted">影响四层 Bloom Filter 的容量规模，实际值会按运行档位归一化。</p>
            </label>
          </div>
        </CyberCard>

        <div class="space-y-6">
          <CyberCard title="运行摘要" sub-title="帮助你快速确认当前实例到底在按什么模式跑。">
            <div class="space-y-4 text-sm text-stone-700">
              <div class="rounded-[24px] border border-cyber-border/60 p-4">
                <div class="flex items-center justify-between gap-4">
                  <div>
                    <p class="text-xs tracking-[0.16em] text-cyber-muted">当前运行状态</p>
                    <p class="mt-2 text-lg font-semibold text-stone-900">{{ runtimeStatus ? 'L4 检测实例已加载' : 'L4 检测实例未加载' }}</p>
                  </div>
                  <ServerCog class="text-cyber-accent-strong" :size="22" />
                </div>
                <p class="mt-3 leading-6 text-cyber-muted">
                  运行时统计来自内存中的 L4 Inspector，保存配置后如果不重启，这里的统计仍然对应旧参数。
                </p>
              </div>

              <div class="grid gap-4 sm:grid-cols-2">
                <div class="rounded-[24px] border border-cyber-border/60 p-4">
                  <p class="text-xs tracking-[0.16em] text-cyber-muted">运行档位</p>
                  <p class="mt-2 text-lg font-semibold text-stone-900">{{ runtimeProfileLabel }}</p>
                </div>
                <div class="rounded-[24px] border border-cyber-border/60 p-4">
                  <p class="text-xs tracking-[0.16em] text-cyber-muted">Bloom 状态</p>
                  <p class="mt-2 text-lg font-semibold text-stone-900">{{ meta.bloom_enabled ? '已启用' : '未启用' }}</p>
                </div>
                <div class="rounded-[24px] border border-cyber-border/60 p-4">
                  <p class="text-xs tracking-[0.16em] text-cyber-muted">误判校验</p>
                  <p class="mt-2 text-lg font-semibold text-stone-900">{{ meta.bloom_false_positive_verification ? '开启' : '关闭' }}</p>
                </div>
                <div class="rounded-[24px] border border-cyber-border/60 p-4">
                  <p class="text-xs tracking-[0.16em] text-cyber-muted">配置生效方式</p>
          ...
          <CyberCard title="Bloom 摘要" sub-title="如果四层 Bloom 已启用，这里能直接看到三个过滤器的命中概览。">
            <div v-if="bloomPanels.length" class="space-y-4">
              ...
              >
                <div class="flex items-center justify-between gap-4">
                  ...
                  <StatusBadge :text="`${(item.value.hit_rate * 100).toFixed(2)}%`" type="info" compact />
                </div>
                <div class="mt-3 grid grid-cols-2 gap-3 text-xs text-cyber-muted">
      ...
      <CyberCard title="端口维度统计" sub-title="优先按拦截量、DDoS 事件和连接量排序，便于快速看出热点端口。" no-padding>
        <div v-if="topPorts.length" class="overflow-x-auto">
          <table class="min-w-full border-collapse text-left">
            <thead class="bg-cyber-surface-strong text-sm text-cyber-muted">
              <tr>
                <th class="px-6 py-4 font-medium">目标端口</th>
                <th class="px-6 py-4 font-medium">连接数</th>
                <th class="px-6 py-4 font-medium">拦截数</th>
                <th class="px-6 py-4 font-medium">DDoS 事件</th>
                <th class="px-6 py-4 font-medium">处理字节</th>
                <th class="px-6 py-4 font-medium">关注度</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                <td class="px-6 py-4 font-mono font-semibold">{{ item.port }}</td>
                <td class="px-6 py-4">{{ formatNumber(item.connections) }}</td>
                <td class="px-6 py-4">{{ formatNumber(item.blocks) }}</td>
                <td class="px-6 py-4">{{ formatNumber(item.ddos_events) }}</td>
                <td class="px-6 py-4">{{ formatNumber(item.bytes_processed) }}</td>
                <td class="px-6 py-4">
                  <StatusBadge
                    :text="item.blocks > 0 || item.ddos_events > 0 ? '重点关注' : '流量观测'"
                    :type="item.blocks > 0 || item.ddos_events > 0 ? 'warning' : 'muted'"
                    compact
                  />
                </td>
              </tr>
        ...
        <div v-else class="px-6 py-10 text-center text-sm text-cyber-muted">
      ...
      <section class="grid gap-6 lg:grid-cols-3">
        <CyberCard title="限流阈值" sub-title="用于快速复核当前保存的关键阈值。">
          <div class="space-y-3 text-sm text-stone-700">
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>每秒连接阈值</span>
              <span class="font-mono font-semibold text-stone-900">{{ formatNumber(configForm.connection_rate_limit) }}</span>
            </div>
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>突发判定阈值</span>
              <span class="font-mono font-semibold text-stone-900">{{ formatNumber(configForm.syn_flood_threshold) }}</span>
            </div>
          </div>
        </CyberCard>
        ...
        <CyberCard title="容量上限" sub-title="帮助判断跟踪表和封禁表的容量预估。">
          <div class="space-y-3 text-sm text-stone-700">
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>跟踪 IP 上限</span>
              <span class="font-mono font-semibold text-stone-900">{{ formatNumber(configForm.max_tracked_ips) }}</span>
            </div>
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>封禁表上限</span>
        ...
        <CyberCard title="清理策略" sub-title="维护任务会按这个 TTL 回收过期状态。">
          <div class="space-y-3 text-sm text-stone-700">
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>状态 TTL</span>
              <span class="font-mono font-semibold text-stone-900">{{ formatNumber(configForm.state_ttl_secs) }} 秒</span>
            </div>
            <div class="flex items-center justify-between rounded-[18px] bg-cyber-surface-strong px-4 py-3">
              <span>Bloom 缩放</span>
