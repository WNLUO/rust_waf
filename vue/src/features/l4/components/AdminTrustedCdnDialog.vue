  ...
  >
    ...
    >
      <div class="flex items-start justify-between gap-4">
        <div>
          ...
          <p class="mt-2 text-sm leading-6 text-slate-500">
            这里配置的手动网段和厂商同步结果会并入 L4 可信来源列表，同时参与真实来源识别链路。
          </p>
        </div>
        <button
      ...
      <div class="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1.05fr)_minmax(0,1fr)]">
        <section class="space-y-3 rounded-2xl border border-slate-200 bg-white/90 p-4">
          <div class="flex items-center justify-between gap-3">
            ...
            <span class="rounded-full border border-blue-100 bg-blue-50 px-3 py-1 text-xs text-blue-700">
          ...
          <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
            ...
            <p class="mt-2 text-xs leading-5 text-slate-500">
              当前会生效 {{ effectivePreviewCount }} 条可信 CDN
              IP/IP 段。保存后 5 秒内会按配置触发首次自动同步。
            </p>
          </div>
        </section>

        <section class="space-y-4 rounded-2xl border border-slate-200 bg-white/90 p-4">
          <div class="grid gap-3 md:grid-cols-[minmax(0,1fr)_10rem_9rem]">
            <label class="space-y-1.5">
              <span class="text-xs font-medium text-slate-500">自动同步周期</span>
              <input
            ...
            <div class="space-y-1.5">
              ...
              <div class="rounded-[16px] border border-slate-200 bg-slate-50 px-3.5 py-2.5 text-xs leading-5 text-slate-500">
                所有开启的内置厂商共用这个周期。
              </div>
            </div>
          </div>
          ...
          <section class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
              <span
                ...
                :class="statusClass(props.form.trusted_cdn.edgeone_overseas.last_sync_status)"
              >
                {{ statusLabel(props.form.trusted_cdn.edgeone_overseas.last_sync_status) }}
              </span>
              <span class="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-600">
                已同步 {{ props.form.trusted_cdn.edgeone_overseas.synced_cidrs.length }} 条
              </span>
              <span class="text-slate-500">
                最近同步：{{ formatSyncTime(props.form.trusted_cdn.edgeone_overseas.last_synced_at) }}
              </span>
            </div>
          ...
          <section class="rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
            <div class="flex items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-xs leading-5 text-slate-500">
                  通过 ESA 官方 OpenAPI 获取站点回源白名单，需填写站点 ID 和 AccessKey。
                </p>
              </div>
              <label
                class="inline-flex items-center gap-2 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs text-stone-700"
            ...
            <div class="mt-3 grid gap-3 md:grid-cols-2">
              ...
              <label class="space-y-1.5">
                <span class="text-xs font-medium text-slate-500">API Endpoint</span>
                <input
              ...
              <label class="space-y-1.5">
                <span class="text-xs font-medium text-slate-500">AccessKey ID</span>
                <input
              ...
              <label class="space-y-1.5">
                <span class="text-xs font-medium text-slate-500">AccessKey Secret</span>
                <input
            ...
            <div class="mt-3 flex flex-wrap items-center gap-2 text-xs">
              <span
                class="rounded-full border px-3 py-1"
                :class="statusClass(props.form.trusted_cdn.aliyun_esa.last_sync_status)"
              >
                {{ statusLabel(props.form.trusted_cdn.aliyun_esa.last_sync_status) }}
              </span>
              <span class="rounded-full border border-slate-200 bg-white px-3 py-1 text-slate-600">
                已同步 {{ props.form.trusted_cdn.aliyun_esa.synced_cidrs.length }} 条
              </span>
              <span class="text-slate-500">
                最近同步：{{ formatSyncTime(props.form.trusted_cdn.aliyun_esa.last_synced_at) }}
              </span>
            </div>
...
.ui-switch {
  ...
  background: linear-gradient(135deg, rgba(148, 163, 184, 0.8), rgba(100, 116, 139, 0.75));
  position: relative;
  outline: none;
  cursor: pointer;
  transition:
