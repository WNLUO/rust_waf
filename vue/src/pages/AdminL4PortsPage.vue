...
const filters = reactive({
  ...
  sort_by: 'blocks' as 'blocks' | 'connections' | 'ddos_events' | 'bytes_processed' | 'port',
...
const sortedPorts = computed(() => {
  const items = [...(stats.value?.per_port_stats ?? [])]
    .filter((item) => {
  ...
  items.sort((left, right) => {
    ...
    const compare = (() => {
      switch (filters.sort_by) {
        ...
        case 'port':
          return left.port.localeCompare(right.port, 'zh-CN', { numeric: true })
        case 'blocks':
        default:
...
const topByConnections = computed(() =>
  [...(stats.value?.per_port_stats ?? [])].sort((left, right) => right.connections - left.connections)[0] ?? null,
)
const topByBlocks = computed(() =>
  [...(stats.value?.per_port_stats ?? [])].sort((left, right) => right.blocks - left.blocks)[0] ?? null,
)
const ddosPorts = computed(() => (stats.value?.per_port_stats ?? []).filter((item) => item.ddos_events > 0))
const totalBytes = computed(() =>
  (stats.value?.per_port_stats ?? []).reduce((sum, item) => sum + item.bytes_processed, 0),
)
    ...
    <div class="space-y-6">
      ...
      <section class="rounded-2xl border border-cyber-border/60 bg-white p-6 shadow-sm">
        ...
        <p class="mt-4 max-w-3xl text-sm leading-7 text-stone-700">
          这里直接读取 L4 Inspector 的端口级统计，适合值班时快速定位被重点扫描、被频繁拦截或出现洪泛特征的入口端口。
        </p>
      </section>
      ...
      <section class="grid grid-cols-1 gap-4 md:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricWidget
          ...
          :hint="topByConnections ? `连接 ${formatNumber(topByConnections.connections)}` : '暂无数据'"
          :icon="Activity"
        />
        <MetricWidget
          label="最高拦截端口"
          :value="topByBlocks?.port || '暂无'"
          :hint="topByBlocks ? `拦截 ${formatNumber(topByBlocks.blocks)}` : '暂无数据'"
          :icon="AlertTriangle"
          trend="up"
        />
        <MetricWidget
      ...
      <section class="grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <CyberCard title="热点摘要" sub-title="帮助你先盯最值得看的几个端口。">
          <div class="space-y-4">
            <div class="rounded-[24px] border border-cyber-border/60 bg-cyber-surface-strong p-4">
              <p class="text-xs tracking-[0.18em] text-cyber-muted">连接最活跃</p>
              ...
              <p class="mt-2 text-sm text-cyber-muted">
                {{ topByConnections ? `累计连接 ${formatNumber(topByConnections.connections)}` : '还没有足够的端口统计数据。' }}
              </p>
            </div>
            <div class="rounded-[24px] border border-cyber-border/60 bg-cyber-surface-strong p-4">
              <p class="text-xs tracking-[0.18em] text-cyber-muted">拦截最密集</p>
              <p class="mt-2 text-2xl font-semibold text-stone-900">
                {{ topByBlocks ? `${topByBlocks.port} 端口` : '暂无' }}
              </p>
              <p class="mt-2 text-sm text-cyber-muted">
                {{ topByBlocks ? `拦截 ${formatNumber(topByBlocks.blocks)} 次 / DDoS ${formatNumber(topByBlocks.ddos_events)} 次` : '暂无拦截热点。' }}
              </p>
            </div>
            <div class="rounded-[24px] border border-cyber-border/60 bg-cyber-surface-strong p-4">
              <p class="text-xs tracking-[0.18em] text-cyber-muted">DDoS 命中端口</p>
              <div class="mt-3 flex flex-wrap gap-2">
                <StatusBadge
                  ...
                  compact
                />
                <span v-if="!ddosPorts.length" class="text-sm text-cyber-muted">暂无 DDoS 热点。</span>
              </div>
            </div>
          </div>
        </CyberCard>
          ...
          <div class="grid gap-4 md:grid-cols-2">
            <label class="text-sm text-stone-700">
              ...
              <input
                ...
                type="text"
                class="mt-2 w-full rounded-[18px] border border-cyber-border/70 bg-white px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-cyber-accent/40"
                placeholder="例如 22 / 443 / 8080"
              />
            </label>
            <label class="text-sm text-stone-700">
              <span class="font-medium text-stone-900">聚焦视角</span>
              <select v-model="filters.focus" class="mt-2 w-full rounded-[18px] border border-cyber-border/70 bg-white px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-cyber-accent/40">
            ...
            <label class="text-sm text-stone-700">
              ...
              <select v-model="filters.sort_by" class="mt-2 w-full rounded-[18px] border border-cyber-border/70 bg-white px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-cyber-accent/40">
            ...
            <label class="text-sm text-stone-700">
              ...
              <select v-model="filters.sort_direction" class="mt-2 w-full rounded-[18px] border border-cyber-border/70 bg-white px-4 py-3 text-sm text-stone-800 outline-none transition focus:border-cyber-accent/40">
          ...
          <table class="min-w-full border-collapse text-left">
            <thead class="bg-cyber-surface-strong text-sm text-cyber-muted">
              <tr>
                ...
                <th class="px-6 py-4 font-medium">画像标签</th>
              </tr>
            </thead>
            <tbody>
              <tr
              ...
              >
                ...
                <td class="px-6 py-4">
                  <StatusBadge :text="focusBadge(item).text" :type="focusBadge(item).type" compact />
                </td>
              </tr>
              <tr v-if="!sortedPorts.length && !loading">
                <td colspan="6" class="px-6 py-10 text-center text-sm text-cyber-muted">当前筛选条件下没有端口画像数据。</td>
              </tr>
            </tbody>
          </table>
        </div>
