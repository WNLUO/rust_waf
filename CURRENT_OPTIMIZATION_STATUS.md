...
| 攻击 + 正常业务混合流量下 2000+ | 60s / 180s 已过第一目标，600s 待验证 | Stage 12 的 95/5、90/10、80/20 / 2200 RPS / 60s 正常成功率分别为 `95.76%`、`95.10%`、`94.78%`；修复 blocked IP 持久化误伤后，180s 达到 `95.86%`、`97.55%`、`98.84%` |

因此，准确说法是：

```text
当前已经在 2 核 / 512MB 环境下，用 multi closed-loop 和 open-loop 口径验证了纯攻击高级 CDN CC 超过 2000 TPS。
Stage 12 已把 95/5、90/10、80/20 混合短测和 180 秒长测正常成功率提升到 95% 左右或以上。
但 600 秒混合长测尚未执行，因此还不能宣称“生产复杂场景下高级 CDN CC 已全面稳定 2000+ TPS”。
```
...
| Stage 12 | 正常用户保活短测第一目标达成；修复 survival fast block 持久化 IP 误伤后，180s 长测达到 `95.86%` / `97.55%` / `98.84%`，600s 待验证 |
...
| 容易混淆的问题 | 清理后的口径 |
|---|---|
| “高级 CC 2000 TPS 是否已实现” | 纯攻击 multi closed-loop 和 open-loop 下已实现；生产混合流量 60s / 180s 已过第一目标，600s 待验证 |
...
下一阶段继续围绕“高压下正常用户是否能长时间存活”：

1. 清空或过期历史 local blocked IP，再重跑 180s，确认剩余 `blocked_client_ip` 是否归零。
2. 执行 600s 95/5、90/10、80/20 混合长测，确认 2 核 / 512MB 下的稳定性。
3. 后续前端接入 early defense / L7 drop reason 分布，便于直接从页面看攻击过程和误伤来源。
4. 继续保持 API 不绕过，避免为了正常 document 保活把 API 攻击也放进来。
5. 600s 通过后，用专业 open-loop 工具复核 2500+ 边界。
...
```text
继续 Stage 12，按 STAGE12_NORMAL_SURVIVAL_REPORT.md 的下一步建议，先清空或过期历史 local blocked IP，然后做 600s 95/5、90/10、80/20 混合长测，确认 180s 通过后的长时间稳定性。
```
