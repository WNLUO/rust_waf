...
function mergeMappingDrafts(siteList: SafeLineSiteItem[], mappingList: SafeLineMappingItem[]) {
  const nextDrafts: SafeLineMappingDraft[] = siteList.map((site) => {
    const existing = mappingList.find((item) => item.safeline_site_id === site.id)
    return {
...
const hasSavedConfig = computed(() => Boolean(settings.value?.safeline.base_url.trim()))

const authMode = computed(() => {
  if (settings.value?.safeline.api_token.trim()) return 'API Token'
  if (settings.value?.safeline.username.trim() && settings.value?.safeline.password.trim()) {
    return '账号密码'
  }
  return '未配置鉴权'
})
...
async function loadPageData() {
  ...
  try {
    const [settingsResponse, mappingsResponse, syncResponse] = await Promise.all([
      fetchSettings(),
      fetchSafeLineMappings(),
      fetchSafeLineSyncState(),
    ])
  ...
  <AppLayout>
    ...
    <div class="space-y-6">
      ...
      <template v-else>
        <section class="grid gap-6 xl:grid-cols-[1fr_1.1fr] xl:items-start">
          <div class="rounded-[30px] border border-white/80 bg-white/78 p-6 shadow-[0_14px_40px_rgba(90,60,30,0.07)]">
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2">
              <div class="rounded-[20px] bg-cyber-surface-strong p-4">
                <p class="text-xs text-cyber-muted">雷池地址</p>
                <p class="mt-2 break-all text-sm font-medium text-stone-900">
                  {{ settings?.safeline.base_url || '未配置' }}
                </p>
              </div>
              <div class="rounded-[20px] bg-cyber-surface-strong p-4">
                <p class="text-xs text-cyber-muted">鉴权方式</p>
                <p class="mt-2 text-sm font-medium text-stone-900">{{ authMode }}</p>
              </div>
              <div class="rounded-[20px] bg-cyber-surface-strong p-4">
                <p class="text-xs text-cyber-muted">站点列表路径</p>
                <p class="mt-2 break-all font-mono text-xs text-stone-900">
                  {{ settings?.safeline.site_list_path || '未配置' }}
                </p>
              </div>
              <div class="rounded-[20px] bg-cyber-surface-strong p-4">
                <p class="text-xs text-cyber-muted">事件列表路径</p>
                <p class="mt-2 break-all font-mono text-xs text-stone-900">
            ...
            >
              <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <StatusBadge
                  :text="testResult.status === 'ok' ? '通过' : testResult.status === 'warning' ? '需确认' : '失败'"
                  :type="testResult.status === 'ok' ? 'success' : testResult.status === 'warning' ? 'warning' : 'error'"
                  compact
                />
              </div>
          ...
          <div class="rounded-[30px] border border-white/80 bg-white/78 p-6 shadow-[0_14px_40px_rgba(90,60,30,0.07)]">
            <div class="flex items-center justify-between gap-3">
              <div>
                ...
                <p class="mt-1 text-xs leading-5 text-cyber-muted">这些按钮直接调用后端已实现的雷池同步接口。</p>
              </div>
              <ShieldCheck :size="18" class="text-cyber-accent-strong" />
            </div>
            ...
            <div class="mt-4 grid gap-3 md:grid-cols-2">
              <button
                ...
                class="flex items-center justify-between rounded-[20px] border border-cyber-border/70 bg-cyber-surface-strong px-4 py-3 text-left transition hover:border-cyber-accent/40 disabled:cursor-not-allowed disabled:opacity-60"
              >
                <span>
                  <span class="block text-sm font-medium text-stone-900">同步雷池事件</span>
                  <span class="mt-1 block text-xs text-cyber-muted">写入本地事件库并套用站点映射。</span>
                </span>
                <RefreshCw :size="16" :class="{ 'animate-spin': actions.syncingEvents }" />
              </button>
              ...
              >
                <span>
                  <span class="block text-sm font-medium text-stone-900">拉取雷池封禁</span>
                  <span class="mt-1 block text-xs text-cyber-muted">把远端封禁同步到本地封禁名单。</span>
                </span>
                <Download :size="16" />
              </button>
              ...
              >
                <span>
                  <span class="block text-sm font-medium text-stone-900">推送本地封禁</span>
                  <span class="mt-1 block text-xs text-cyber-muted">把本地封禁联动到雷池。</span>
                </span>
                <Upload :size="16" />
              </button>
              ...
              >
                <span>
                  <span class="block text-sm font-medium text-stone-900">刷新执行状态</span>
                  <span class="mt-1 block text-xs text-cyber-muted">查看最近一次成功时间和导入统计。</span>
                </span>
            ...
            <div class="mt-5 grid gap-3">
              <article
                ...
                :key="item.key"
                class="rounded-[20px] border border-cyber-border/60 bg-white/80 p-4"
              >
                <div class="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    <p class="text-sm font-medium text-stone-900">{{ item.title }}</p>
                    <p class="mt-1 text-xs leading-5 text-cyber-muted">{{ item.description }}</p>
                  </div>
                  <StatusBadge :text="syncStatusText(item.data)" :type="syncStatusType(item.data)" compact />
                </div>
                <div class="mt-3 grid gap-2 text-xs text-cyber-muted md:grid-cols-2">
        ...
        <section class="rounded-[30px] border border-white/80 bg-white/78 p-6 shadow-[0_14px_40px_rgba(90,60,30,0.07)]">
          <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            ...
            <div class="flex flex-wrap gap-2">
              <button
                @click="clearPrimary"
                class="inline-flex items-center gap-1.5 rounded-lg border border-cyber-border/70 bg-white px-3 py-1.5 text-xs font-medium text-stone-700 transition hover:border-cyber-accent/40 hover:text-cyber-accent-strong"
              >
                清空主站点
              </button>
              <button
              ...
              <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                <div class="space-y-2">
                  <div class="flex flex-wrap items-center gap-2">
                    <p class="text-sm font-medium text-stone-900">{{ draft.safeline_site_name || '未命名站点' }}</p>
                    <StatusBadge v-if="draft.enabled" text="启用映射" type="success" compact />
                    <StatusBadge v-else text="停用映射" type="muted" compact />
                    <StatusBadge v-if="draft.is_primary" text="主站点" type="info" compact />
                    <StatusBadge v-if="draft.orphaned" text="历史映射" type="warning" compact />
                  </div>
                  <p class="font-mono text-xs text-cyber-muted">站点 ID：{{ draft.safeline_site_id }}</p>
                  <p class="font-mono text-xs text-cyber-muted">域名：{{ draft.safeline_site_domain || '未提供域名' }}</p>
                  <p class="text-xs text-cyber-muted">上次更新：{{ formatTimestamp(draft.updated_at) }}</p>
                </div>
