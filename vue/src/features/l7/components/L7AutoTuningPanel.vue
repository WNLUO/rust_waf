  ...
  <div class="mt-4 border-t border-slate-200 pt-4">
    ...
    >
      ...
      <p class="mt-1">
        观测值: 握手超时率
        {{
          runtime.last_observed_tls_handshake_timeout_rate_percent.toFixed(2)
        }}% / 预算拒绝率
        {{ runtime.last_observed_bucket_reject_rate_percent.toFixed(2) }}% /
        平均代理延迟 {{ runtime.last_observed_avg_proxy_latency_ms }}ms
      </p>
      <p class="mt-1">
        最近动作:
        {{ helpers.adjustReasonLabel(runtime.last_adjust_reason) }} | 24h 回滚:
        {{ runtime.rollback_count_24h }}
      </p>
      ...
      <template v-if="controls.autoEffectEvaluation.value">
        ...
        <p class="mt-1">
          ...
          {{
            helpers.formatSignedNumber(
              controls.autoEffectEvaluation.value
        ...
        <p class="mt-1">
          说明: {{ controls.autoEffectEvaluation.value.summary }}
        </p>
        <p
          v-if="controls.autoEffectEvaluation.value.segments.length"
          class="mt-1"
        >
          分层观测:
          {{
        ...
        <div
          v-if="
            controls.autoRiskByHost.value.length ||
            controls.autoRiskByRoute.value.length
          "
        ...
        >
          ...
          <div class="mt-2 grid gap-2 md:grid-cols-2 xl:grid-cols-3">
            ...
            >
              ...
              <p class="mt-1 text-[11px] leading-5">
                热度
                {{ helpers.formatSignedInteger(item.max_latency_delta_ms) }}ms /
                {{
                  helpers.formatSignedNumber(
                    item.max_failure_rate_delta_percent,
                  )
                }}pp
              </p>
