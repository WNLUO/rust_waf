...
| 是否已经解决混合正常流量 | 95/5、90/10、80/20 短测均已过 90%；80/20 为 `94.78%`，略低于 95% 线，仍需长测和继续压低无响应 |
| 是否影响 API 攻击防护 | 单测确认 API 伪造身份不会绕过 fast block |
| 下一步优先级 | 补 180s / 600s 长测，并继续降低剩余无响应和 L4 degrade 影响 |

## 6. 下一步建议

1. 补 normal-only 50 RPS 阶梯点，和 100/200 RPS 形成更完整曲线。
